...
OKCupid.prototype.login = function(username, password, callback){
    ...
    this.requester.postRequest(URLS.login, data, this._extractAccessTokenAndCallCallback(callback))
};

OKCupid.prototype.visitUser =  function(username, callback){
    var user_profile_url = URLS.visit_user.replace('{username}', username)
    this.requester.getRequest(user_profile_url, this._extractAccessTokenAndCallCallback(callback))
};
...
OKCupid.prototype.rate = function(target_userid, score, callback){
    ...
    this.requester.postRequest(URLS.rate, data, this._extractAccessTokenAndCallCallback(callback))
};

OKCupid.prototype.getQuickmatch = function(callback){
    this.requester.getRequest(URLS.quickmatch, this._extractAccessTokenAndCallCallback(callback))
};
...
OKCupid.prototype.getUserAnswers = function(username, offset, callback){
    ...
    this.requester.getRequest(user_answers_url, this._extractAccessTokenAndCallCallback(callback))
};

OKCupid.prototype.getVisitors = function(callback){
    this.requester.getRequest(URLS.get_visitors, this._extractAccessTokenAndCallCallback(callback))
};
...
OKCupid.prototype.getConversations = function(callback, offset) {
    ...
    this.requester.getRequest(messagesUrl, function(error, response, body) {
        if(body.messages.length > 0) {
            ...
            that.getConversations(that._extractAccessTokenAndCallCallback(callback), offset + MESSAGES_PER_REQUEST)
        }
        else {
...
OKCupid.prototype.getConversation = function(conversationId, callback) {
    ...
    return this.requester.getRequest(conversationUrl, this._extractAccessTokenAndCallCallback(callback));
};

OKCupid.prototype.sendMessage = function(receiverId, threadId, body, callback) {
    ...
    console.log(data);
    return this.requester.postRequest(sendMessageUrl, JSON.stringify(data), this._extractAccessTokenAndCallCallback(callback))
};

OKCupid.prototype.deleteConversation = function(conversationId, callback) {
    var deleteUrl = URLS.deleteConversation.replace('{conversation}', conversationId).replace('{access_token}', this.accessToken);
    return this.requester.getRequest(deleteUrl, this._extractAccessTokenAndCallCallback(callback));
}

OKCupid.prototype._extractAccessTokenAndCallCallback = function(callback) {
    var that = this;
    return function(error, response, body) {
        if(body && body.authcode) {
            that.accessToken = body.authcode;
        }
